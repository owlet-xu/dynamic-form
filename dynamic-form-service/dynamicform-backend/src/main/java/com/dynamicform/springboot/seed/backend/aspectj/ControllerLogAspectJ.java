...
public class ControllerLogAspectJ {
    ...
    //  前置通知：
    @Before("execution(public *  com.dynamicform.springboot.seed.bz1.controller..*.*(..))")
    ...
    @After("execution(public *  com.dynamicform.springboot.seed.bz1.controller..*.*(..))")
    public void logAfter(JoinPoint joinPoint) {
        logger.info("The method {} end",joinPoint.getSignature().getName());
    }

    /*异常通知：仅当连接点抛出异常时执行。*/
    @AfterThrowing(pointcut = "execution(public *  com.dynamicform.springboot.seed.bz1.controller..*.*(..))", throwing = "throwable")
